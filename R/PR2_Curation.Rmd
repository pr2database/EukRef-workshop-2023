...
pr2 <- pr2_database()
Order   = pr2 %>% dplyr::filter(order   == "Suessiales") %>% dplyr::select(pr2_accession, sequence_length, sequence)
seq_clade(Order)
```
...
- Just filter according to your lineage of interest and create a new tab-deliminated file with the pr2 accession in column 1 and taxonomy in the following columns
...
library(writexl)
library(dplyr)
# Read the original sequences and their taxonomic annotations from the Excel file. This file is derived from merged pr2 filtered to your lineage
original_sequences_file <- "lineage.xlsx"  
original_sequences_df <- read_excel(original_sequences_file)
...
```{r}
edited_file <- "/Volumes/GoogleDrive/My Drive/EukRef_Barcelona2023/representatives.xlsx"  # Update with the path to your edited Excel file
edited_representatives_df <- read_excel(edited_file)
representatives <- unique(cluster_df$V10)
representatives_df <- original_sequences_df[original_sequences_df$pr2_accession %in% representatives, ]

# Select the relevant columns from edited_representatives_df
update_cols <- c("genbank_accession", "domain", "supergroup", "division", "subdivision", "class", "order", "family", "genus", "species")
